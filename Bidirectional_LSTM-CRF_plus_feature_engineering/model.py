...
class Model(object):
    ...
    def __init__(self, parameters=None, models_path=None, model_path=None):
        ...
        if model_path is None:
            ...
            self.name = "1_CCE_noCRF_LSTMchar25_LSTMword100_Dropout0.5_TagIOB_LearnSGD_glove_mimic_600_features_double"
            # Model location
    ...
    def save_mappings(self, id_to_word, id_to_char, id_to_POStag, id_to_cluster, id_to_tag):
        """
    ...
    def build(self,
        ...
        if semantic_features_dim:
            n_POStags = len(self.id_to_POStag)
            n_lemmas=2
            n_metamap=2
            n_clusters=len(self.id_to_cluster)
        ...
        if morph_features_dim:
            ...
            ends_s_ids = T.ivector(name='ends_s_ids')
            digit_w_ids = T.ivector(name='digit_w_ids')
            numeric_ids = T.ivector(name='numeric_ids')
            alpha_ids = T.ivector(name='alpha_ids')
            alphanum_ids = T.ivector(name='alphanum_ids')
            stop_w_ids = T.ivector(name='stop_w_ids')
        if semantic_features_dim:
            POStag_ids = T.ivector(name='POStag_ids')
            lemma_ids = T.ivector(name='lemma_ids')
            metamap_ids = T.ivector(name='metamap_ids')
            cluster_ids = T.ivector(name='cluster_ids')
        ...
        if morph_features_dim:
            #Capitalization
            input_dim += 4
            cap_layer = EmbeddingLayer(n_cap, 4, name='cap_layer')
            inputs.append(cap_layer.link(cap_ids))
            #Ends in s
            input_dim += 4
            ends_s_layer = EmbeddingLayer(n_ends_s, 4, name='ends_s_layer')
            inputs.append(ends_s_layer.link(ends_s_ids))
            #Has digit in the string
            input_dim += 4
            digit_w_layer = EmbeddingLayer(n_digit_w, 4, name='digit_w_layer')
            inputs.append(digit_w_layer.link(digit_w_ids))
            #String is a number
            input_dim += 4
            numeric_layer = EmbeddingLayer(n_numeric, 4, name='numeric_layer')
            inputs.append(numeric_layer.link(numeric_ids))
            #String is just alphabetic
            input_dim += 4
            alpha_layer = EmbeddingLayer(n_alpha, 4, name='alpha_layer')
            inputs.append(alpha_layer.link(alpha_ids))
            #String is just alphanumeric
            input_dim += 4
            alphanum_layer = EmbeddingLayer(n_alphanum, 4, name='alphanum_layer')
            inputs.append(alphanum_layer.link(alphanum_ids))
            #The word is a stopword
            input_dim += 4
            stop_w_layer = EmbeddingLayer(n_stop_w, 4, name='stop_w_layer')
            inputs.append(stop_w_layer.link(stop_w_ids))
        ...
        if semantic_features_dim:
            #Lemmas
            input_dim += 4
            lemma_layer = EmbeddingLayer(n_lemmas,4, name='lemma_layer')
            inputs.append(lemma_layer.link(lemma_ids))
            #Metamap
            input_dim += 4
            metamap_layer = EmbeddingLayer(n_metamap,4, name='metamap_layer')
            inputs.append(metamap_layer.link(metamap_ids))
            #POS tags
            input_dim += 70
            POStags_layer = EmbeddingLayer(n_POStags,70, name='POStags_layer')
            inputs.append(POStags_layer.link(POStag_ids))
            #Clusters
            input_dim += 40
            cluster_layer = EmbeddingLayer(n_clusters,40,name='cluster_layer')
            inputs.append(cluster_layer.link(cluster_ids))
        ...
        if morph_features_dim:
            ...
            self.add_component(ends_s_layer)
            params.extend(ends_s_layer.params)
            self.add_component(digit_w_layer)
            params.extend(digit_w_layer.params)
            self.add_component(numeric_layer)
            params.extend(numeric_layer.params)
            self.add_component(alpha_layer)
            params.extend(alpha_layer.params)
            self.add_component(alphanum_layer)
            params.extend(alphanum_layer.params)
            self.add_component(stop_w_layer)
            params.extend(stop_w_layer.params)
        if semantic_features_dim:
            self.add_component(lemma_layer)
            params.extend(lemma_layer.params)
            self.add_component(metamap_layer)
            params.extend(metamap_layer.params)
            self.add_component(POStags_layer)
            params.extend(POStags_layer.params)
            self.add_component(cluster_layer)
            params.extend(cluster_layer.params)
        ...
        if morph_features_dim:
            ...
            eval_inputs.append(ends_s_ids)
            eval_inputs.append(digit_w_ids)
            eval_inputs.append(numeric_ids)
            eval_inputs.append(alpha_ids)
            eval_inputs.append(alphanum_ids)
            eval_inputs.append(stop_w_ids)
        if semantic_features_dim:
            eval_inputs.append(lemma_ids)
            eval_inputs.append(metamap_ids)
            eval_inputs.append(POStag_ids)
            eval_inputs.append(cluster_ids)
