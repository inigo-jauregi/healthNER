...
optparser.add_option(
    "-p", "--pre_emb", default="pre_trainedVectorEmbeddings/pre_emb_glove_mimic_600.txt",
...
for epoch in xrange(n_epochs):
    ...
    for i, index in enumerate(np.random.permutation(len(train_data))):
        ...
        if count % freq_eval == 0:
            ...
            store_dev.append(dev_score)
            store_test.append(test_score)
            ...
            if dev_score > best_dev:
                ...
                test_lines_with_best_dev=test_eval_lines_prob
...
print "Stored history in CSV"
csvfileTest=open("results/test_history_adadelta_1.csv","w")
csv_writerTest=csv.writer(csvfileTest,delimiter=',',quotechar='"',lineterminator='\n')
csv_writerTest.writerow(store_test)
csvfiledev=open("results/dev_history_adadelta_1.csv","w")
csv_writerdev=csv.writer(csvfiledev,delimiter=',',quotechar='"',lineterminator='\n')
csv_writerdev.writerow(store_dev)

final_dev_lines=""
for line in best_dev_eval_lines:
    final_dev_lines=final_dev_lines+"\n"+line
final_test_lines_with_best_dev=""
for line in test_lines_with_best_dev:
    final_test_lines_with_best_dev=final_test_lines_with_best_dev+"\n"+line
filename=open('results/20_dev_scores_MedLine_glove_fixed_mimic_600.txt','w')
filename.write(str(final_dev_lines))
filename.write("\n\nTest with best dev: "+str(test_with_best_dev))
filename.write("\n\nTest lines with best dev: "+str(final_test_lines_with_best_dev))
filename.close()

final_test_lines=""
for line2 in best_test_eval_lines:
    final_test_lines=final_test_lines+"\n"+line2
filename=open('results/20_test_scores_MedLine_glove_fixed_mimic_600.txt','w')
filename.write(final_test_lines)
filename.close()

#I need to create a text describing the experiments in the results
filename=open('results/20_MedLine_glove_fixed_mimic_600_description.txt','w')
filename.write('Experiment 20:\n')
filename.write('-MedLine\n')
filename.write('-Bidirectional LSTM-CRF\n')
filename.write('-Glove fixed + mimic (600) \n')
