...
def create_input(data, parameters, add_label, singletons=None):
    ...
    if parameters['morph_features_dim']:
        caps = data['caps']
        ends_s= data['ends_s']
        digit_w= data['digit_w']
        numeric= data['numeric']
        alpha= data['alpha']
        alphanum= data['alphanum']
        stop_w= data['stop_w']
    if parameters['semantic_features_dim']:
        lemmas=data['lemma']
    ...
    if parameters['morph_features_dim']:
        input.append(caps)
        input.append(ends_s)
        input.append(digit_w)
        input.append(numeric)
        input.append(alpha)
        input.append(alphanum)
        input.append(stop_w)
    if parameters['semantic_features_dim']:
        input.append(lemmas)
        input.append(metamap)
        input.append(POStag)
        input.append(clusters)
    if add_label:
...
def evaluate(parameters, f_eval, raw_sentences, parsed_sentences,
    ...
    # F1 on all entities
    return float(eval_lines[1].strip().split()[-1]), eval_lines
